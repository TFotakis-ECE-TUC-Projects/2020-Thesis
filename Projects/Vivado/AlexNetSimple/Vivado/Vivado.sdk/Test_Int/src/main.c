...
void Conv_core_init(void *InstancePtr) {
	...
	int status =
		XConv_core_Initialize(pAccelerator, XPAR_CONV_CORE_0_DEVICE_ID);
...
void Conv_core_test() {
	...
	for (u32 i = 0; i < X_SIZE; i++) { X_ADDR[i] = X_VALUE; }

	for (u32 i = 0; i < WEIGHTS_SIZE; i++) { WEIGHTS_ADDR[i] = WEIGHT_VALUE; }

	for (u32 i = 0; i < BIAS_SIZE; i++) { BIAS_ADDR[i] = BIAS_VALUE; }

	for (u32 i = 0; i < RES_SIZE; i++) { RES_ADDR[i] = 0; }
	printf("%sSuccess%s\n", KGRN, KNRM);
...
void Maxpool_core_init(void *InstancePtr) {
	...
	XMaxpool_core *pAccelerator = (XMaxpool_core *) InstancePtr;
	int status = XMaxpool_core_Initialize(pAccelerator, XPAR_MAXPOOL_CORE_0_DEVICE_ID);
	if (status != XST_SUCCESS) {
		printf("%sError%s\n", KRED, KNRM);
		exit(XST_FAILURE);
	}
...
void Maxpool_core_test() {
	...
	for (u32 i = 0; i < RES_SIZE; i++) { error += abs(res[i] - pixel_value); }
...
void Linear_core_init(void *InstancePtr) {
	...
	XLinear_core *pAccelerator = (XLinear_core *) InstancePtr;
	int status = XLinear_core_Initialize(pAccelerator, XPAR_LINEAR_CORE_0_DEVICE_ID);
	if (status != XST_SUCCESS) {
		printf("%sError%s\n", KRED, KNRM);
		exit(XST_FAILURE);
	}
