...
typedef int8_t matrix_t;
//typedef ap_uint<128> uint128_t;
typedef unsigned __int128 uint128_t;

#define GENERATE_PARAMS 1
...
u32 askYesDefault(const char *question) {
	char ans;
	do {
...
u32 askNoDefault(const char *question) {
	char ans;
	do {
		printf("%s [y/N]: ", question);
		scanf("%c", &ans);
...
u32 askNumber(const char *question, u32 min, u32 max, u32 defaultAns) {
	//	Max U32: 4294967295
	char ans[11];
	char c;
	u32 num = 0;
...
char *selectFilePath(const char *filesDir, const char *msg) {
...
/**
 * Generates the network's parameters
 * @returns a Params structure pointer containing the loaded network parameters.
 */
matrix_t **generateParameters(NetConf *netConf) {
	printf("- Generating Parameters: ");

	/** Get the number of parameters sets existing in this file */
	u32 params_len = 0;
	for (u32 i = 0; i < netConf->layersNum; i++) {
		if (netConf->layersConf[i].layerType != MAXPOOL_LAYER_TYPE) {
			params_len += 2;
		}
	}
	...
	u32 p = 0;
	for (u32 i = 0; i < netConf->layersNum; i++) {
		LayerConf l = netConf->layersConf[i];
		if (netConf->layersConf[i].layerType != MAXPOOL_LAYER_TYPE) {
			params[p] = (matrix_t *) malloc(l.weightsSize * sizeof(matrix_t));
			if (params[p] == NULL) {
				eraseCharactersTerminal(9);
				printf(
					"%sError. Not enough memory on generateParameters for "
					"params[%d].%s\n",
					KRED,
					p,
					KNRM);
			...
			for (u32 j = 0; j < l.weightsSize; j++) {
				params[p][j] = 1;
			}

			params[p + 1] = (matrix_t *) malloc(l.biasSize * sizeof(matrix_t));
			if (params[p] == NULL) {
				eraseCharactersTerminal(9);
				printf(
					"%sError. Not enough memory on generateParameters for "
...
matrix_t *loadImage(char *path) {
	...
	u32 character, width, height, depth;
	// check for comments
	s = f_gets(buff, sizeof(buff), &f);
	character = buff[0];
	while (character == '#') {
...
void printLayerConf(LayerConf lc) {
	printf("layerType: %d\n", lc.layerType);
	printf("kernelSize: %d\n", lc.kernelSize);
	printf("stride: %d\n", lc.stride);
	printf("padding: %d\n", lc.padding);
	...
	printf("wout: %d\n", lc.wout);
	printf("inFeatures: %d\n", lc.inFeatures);
	printf("outFeatures: %d\n", lc.outFeatures);
	printf("doReLU: %d\n", lc.doReLU);
	printf("xSize: %d\n", lc.xSize);
	printf("weightsSize: %d\n", lc.weightsSize);
	printf("biasSize: %d\n", lc.biasSize);
	printf("resSize: %d\n", lc.resSize);
	printf("weightsAddr: %x\n", (u32) (u64) lc.weightsAddr);
	printf("biasAddr: %x\n", (u32) (u64) lc.biasAddr);
	printf("useInStream: %d\n", lc.useInStream);
	printf("xScale: %d\n", lc.xScale);
	printf("weightsScale: %d\n", lc.weightsScale);
	printf("biasScale: %d\n", lc.biasScale);
	printf("resTopBit: %d\n", lc.resTopBit);
	printf("resDest: %d\n", lc.resDest);
	printf("useOutStream: %d\n", lc.useOutStream);
}

NetConf *selectNetConf(
	const char *configsDir,
	const char *labelsDir,
	const char *paramsDir,
...
#if GENERATE_PARAMS == 0
	netConf->params = loadParameters(paramsFile);
#else
	netConf->params = generateParameters(netConf);
#endif
...
void Conv_core_init(Core *core) {
	...
	int status = XConv_core_Initialize((XConv_core *) core->InstancePtr, Conv_core_device_ids[core->InstanceId]);
...
matrix_t *Conv_core_setup(XConv_core *Conv_core, LayerConf lc, matrix_t *x) {
	printf("Setup...");
	XConv_core_Set_x_V(Conv_core, (u32)(u64) x);
	XConv_core_Set_useInStream(Conv_core, lc.useInStream);
	XConv_core_Set_xScale(Conv_core, lc.xScale);
	XConv_core_Set_weights_V(Conv_core, (u32)(u64) lc.weightsAddr);
	XConv_core_Set_weightsScale(Conv_core, lc.weightsScale);
	XConv_core_Set_bias_V(Conv_core, (u32)(u64) lc.biasAddr);
	XConv_core_Set_biasScale(Conv_core, lc.biasScale);
	...
	XConv_core_Set_doReLU(Conv_core, lc.doReLU);

	u32 diff = sizeof(uint128_t) / sizeof(matrix_t);
	u32 mallocSize = ((lc.resSize / diff) + 1) * diff;
	matrix_t *resAddr = (matrix_t *) malloc(mallocSize * sizeof(matrix_t));
...
//	matrix_t *resAddr = (matrix_t *) malloc(lc.resSize * sizeof(matrix_t));
	...
	XConv_core_Set_res_V(Conv_core, (u32)(u64) resAddr);
	XConv_core_Set_resTopBit(Conv_core, lc.resTopBit);
	XConv_core_Set_resDest(Conv_core, lc.resDest);
	XConv_core_Set_useOutStream(Conv_core, lc.useOutStream);
...
void Conv_conf_params_complete(LayerConf *lc) {
	...
	lc->weightsSize = lc->dout * lc->din * lc->kernelSize * lc->kernelSize;
	lc->biasSize = lc->dout;
...
void Conv_conf_complete(NetConf *netConf, u32 layer_index, u32 params_index) {
	...
	if (layer_index > 0) {
		...
		if (plc->layerType == LINEAR_LAYER_TYPE) {
...
//	printLayerConf(lc);
	...
	for (u32 core = 0; core < core_num; core++) {
		...
		int pixel_value_tmp =
				X_VALUE * WEIGHT_VALUE * lc.kernelSize * lc.kernelSize * lc.din + BIAS_VALUE;
		matrix_t pixel_value = (matrix_t) ((pixel_value_tmp >> (lc.resTopBit - 6)) & 0xFF);
		...
		pixel_value_tmp = X_VALUE * WEIGHT_VALUE * (lc.kernelSize - lc.padding) *
				(lc.kernelSize - lc.padding) * lc.din + BIAS_VALUE;
		pixel_value = (matrix_t) ((pixel_value_tmp >> (lc.resTopBit - 6)) & 0xFF);

		error += fabsf(res[0] - pixel_value);

		pixel_value_tmp = X_VALUE * WEIGHT_VALUE *
...
//		for (u32 i = 0; i < lc.resSize; i++) {
//			printf("Res[%d][%d][%d]: %d\n", i / (lc.hout * lc.wout), (i / lc.wout) % lc.hout, i % lc.wout, res[i]);
//		}

		if(askNoDefault("Print results?")){
			u32 r = 0;
			for (u32 i = 0; i < lc.dout; i++){
...
void Maxpool_core_init(Core *core) {
	...
	int status = XMaxpool_core_Initialize((XMaxpool_core *)
...
matrix_t *
	...
	printf("Setup...");
	XMaxpool_core_Set_x_V(Maxpool_core, (u32)(u64) x);
...
//	matrix_t *resAddr = (matrix_t *) malloc(lc.resSize * sizeof(matrix_t));
	...
	XMaxpool_core_Set_res_V(Maxpool_core, (u32)(u64) resAddr);
	eraseCharactersTerminal(8);
	return resAddr;
}
...
void Linear_core_init(Core *core) {
	...
	int status = XLinear_core_Initialize((XLinear_core *)
...
matrix_t *
	...
	XLinear_core_Set_x_V(Linear_core, (u32)(u64) x);
	XLinear_core_Set_useInStream(Linear_core, lc.useInStream);
	XLinear_core_Set_xScale(Linear_core, lc.xScale);
	XLinear_core_Set_weights_V(Linear_core, (u32)(u64) lc.weightsAddr);
	XLinear_core_Set_weightsScale(Linear_core, lc.weightsScale);
	XLinear_core_Set_bias_V(Linear_core, (u32)(u64) lc.biasAddr);
	XLinear_core_Set_biasScale(Linear_core, lc.biasScale);
...
//	matrix_t *resAddr = (matrix_t *) malloc(lc.resSize * sizeof(matrix_t));
	...
	XLinear_core_Set_res_V(Linear_core, (u32)(u64) resAddr);
	XLinear_core_Set_resTopBit(Linear_core, lc.resTopBit);
	XLinear_core_Set_resDest(Linear_core, lc.resDest);
	XLinear_core_Set_useOutStream(Linear_core, lc.useOutStream);
	eraseCharactersTerminal(8);
	return resAddr;
}
...
int Linear_core_test(u32 testAllCores) {
	...
	u32 resTopBit = askNumber("ResTopBit", 0, 31, 13);
...
//		printf("- Layer %d:\n", i);
//		printLayerConf(netConf->layersConf[i]);
//		printf("-----------------\n");
	}
}
...
int Network_test() {
	printf("*** Network_test ***\n");
	u32 repeat_times = askNumber("Repeat times: ", 1, 1000, 1);
	...
	printf("- Initializing Memory: ");
	u32 sizeDiff = sizeof(uint128_t) / sizeof(matrix_t);
	u32 xSize = lc[0].xSize;
	u32 mallocSize = ((xSize / sizeDiff) + 1) * sizeDiff;
	matrix_t *xAddr = (matrix_t *) malloc(mallocSize * sizeof(matrix_t));
	if (xAddr == NULL) {
		printf(
	...
	for (u32 i = 0; i < LAYERS_NUMBER; i++) {
		...
		mallocSize = ((lc[i].weightsSize / sizeDiff) + 1) * sizeDiff;
		params[params_index] = (matrix_t *) malloc(mallocSize * sizeof(matrix_t));
		...
		mallocSize = ((lc[i].biasSize / sizeDiff) + 1) * sizeDiff;
		params[params_index] = (matrix_t *) malloc(mallocSize * sizeof(matrix_t));
		if (params[params_index] == NULL) {
			printf(
				"%sError. Not enough memory for params[%d] in "
	...
	XTime tStart,tEnd;
	long long unsigned counts;
	long long unsigned avgCounts = 0;
	long long unsigned minCounts = -1;
	long long unsigned maxCounts = 0;
	for (u32 i = 0; i < repeat_times; i++){
		...
		counts = (tEnd - tStart);
		avgCounts += counts;
		minCounts = minCounts > counts ? counts : minCounts;
		maxCounts = maxCounts < counts ? counts : maxCounts;

		free(res);
		layer_conf_complete(&netConf);
	}
	printf("%sSuccess%s\n", KGRN, KNRM);
	avgCounts /= repeat_times;
	printf("Cycles: Avg %llu | Min %llu | Max %llu\n", 2 * avgCounts, 2 * minCounts, 2 * maxCounts);
	float avgTime = 1.0 * avgCounts / COUNTS_PER_SECOND;
	float minTime = 1.0 * minCounts / COUNTS_PER_SECOND;
	float maxTime = 1.0 * maxCounts / COUNTS_PER_SECOND;
	printf("Sec: Avg %f | Min %f | Max %f\n", avgTime, minTime, maxTime);
...
void setup_platform(const char *greeting_message) {
	setup_stdout();
	setup_stdin();
