...
void Conv_conf_complete(
	...
	lc->weightsAddr = netConf->params->matrix[params_index]->matrix;
	lc->biasAddr = netConf->params->matrix[params_index + 1]->matrix;
...
void Maxpool_conf_complete(NetConf *netConf, u32 layer_index, matrix_t *xAddr) {
	LayerConf *lc = &netConf->layersConf[layer_index];
	if (layer_index > 0) {
		LayerConf *plc = &netConf->layersConf[layer_index - 1];
		if (plc->layerType == LINEAR_RELU_LAYER_TYPE ||
...
void Linear_conf_complete(NetConf *netConf, u32 layer_index, u32 params_index, matrix_t *xAddr) {
	LayerConf *lc = &netConf->layersConf[layer_index];
	if (layer_index > 0) {
		LayerConf *plc = &netConf->layersConf[layer_index - 1];
...
void layer_conf_complete(NetConf *netConf, matrix_t *xAddr) {
	u32 params_index = 0;
	for (u32 i = 0; i < netConf->layersNum; i++) {
		switch (netConf->layersConf[i].layerType) {
			case CONV_LAYER_TYPE:
				Conv_conf_complete(netConf, i, params_index, xAddr);
				params_index += 2;
				break;
			case MAXPOOL_LAYER_TYPE:
				Maxpool_conf_complete(netConf, i, xAddr);
				break;
			case LINEAR_RELU_LAYER_TYPE:
			case LINEAR_LAYER_TYPE:
				Linear_conf_complete(netConf, i, params_index, xAddr);
				params_index += 2;
				break;
		}
	}
...
void forward(NetConf *netConf, matrix_t *xAddr) {
	layer_conf_complete(netConf, xAddr);
	for (u32 i = 0; i < netConf->layersNum; i++) {
		printf("- Layer %d\n", i);
		process_layer(netConf->layersConf[i]);
	}
}
